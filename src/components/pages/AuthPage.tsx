    ...
    const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(url, {
                ...
                body: JSON.stringify({ email, password, firstname, sex })
            });
            ...
            if (isLogin && loginUser) {
                loginUser(responseData.user); // Assuming the response contains user data
            ...
            <div className="w-full space-y-6 text-gray-600 sm:max-w-md">
                <div className="text-center">
                    <div className="flex items-center justify-center">
                        <img src={sportrackerLogo} width={50} className="mr-2" alt="Logo" />
                        <span className="text-2xl font-bold text-gray-800">SporTracker</span>
                    </div>
                    <div className="mt-5 space-y-2">
                        <h3 className="text-gray-800 text-2xl font-bold sm:text-3xl">{isLogin ? 'Connectez-vous à votre compte' : 'Créer un compte'}</h3>
                        <p className="">
